...
export default async function ResultadosPage({ params }: { params: { test: string } }) {
  if (!['wcst', 'cinco-puntos', 'af5'].includes(params.test)) notFound()
  ...
  const [{ data: student }, { data: result }] = await Promise.all([
    ...
    (params.test === 'wcst' ? supabase.from('resultados_wcst').select('*') : params.test === 'af5' ? supabase.from('resultados_af5').select('*') : supabase.from('resultados_cinco_puntos').select('*'))
